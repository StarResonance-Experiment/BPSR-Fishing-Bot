...
class Detector:
    ...
    def _perform_match(self, search_area, template_data):
        ...
        if search_gray.shape[0] < template_gray.shape[0] or search_gray.shape[1] < template_gray.shape[1]:
            log(
                f"[DEBUG] ROI too small for template — "
                f"search area: {search_gray.shape[1]}x{search_gray.shape[0]}px, "
                f"template: {template_gray.shape[1]}x{template_gray.shape[0]}px. "
                f"Increase the ROI Width/Height in Detection settings."
            )
            return None, None
    ...
    def find(self, screen, template_name, debug=False):
        ...
        self.last_confidences[template_name] = confidence
